...
public class CloudClientConnector implements ICloudClient, IConnectionListener
{
	...
	public CloudClientConnector() {
		...
		// Initialize the MqttClientConnector with the cloud gateway service section
		this.mqttClient = new MqttClientConnector(true);
		this.mqttClient.setConnectionListener(this);
	}

	/**
	 * Get the MqttClientConnector instance.
	 * 
	 * @return The MqttClientConnector instance.
	 */
	...
	// LED actuation message listener inner class
	private class LedEnablementMessageListener implements IMqttMessageListener
	{
		...
		public void messageArrived(String topic, MqttMessage message) throws Exception
		{
			try {
				...
				// Set the location ID to match the CDA's location ID
	...
	public void onConnect()
	{
		...
		// Create a response actuation event with invalid value to create the topic
		ActuatorData ad = new ActuatorData();
		ad.setAsResponse();
		ad.setName(ConfigConst.LED_ACTUATOR_NAME);
		ad.setValue((float) -1.0); // Invalid actuation value

		String ledTopic = createTopicName(ResourceNameEnum.CDA_ACTUATOR_CMD_RESOURCE.getDeviceName(), 
			ResourceNameEnum.CDA_ACTUATOR_CMD_RESOURCE.getResourceType() + "-" + ad.getName());
	...
	public boolean connectClient() {
		if (this.mqttClient == null) {
			...
			// this.mqttClient = new MqttClientConnector(true);
			// This line is no longer needed here as the mqttClient is initialized in the constructor
